...
trait TestSaves
{
    ...
    private function assertInDatabase(TestResponse $response, array $testDatabaseData)
    {
        ...
        $this->assertDatabaseHas($table, $testDatabaseData + ['affiliate_id' => $this->getIdFromResponse($response)]);
    }
    ...
    private function assertJsonResponseContent(TestResponse $response, array $testDatabaseData, array $testJsonData = null)
    {
        ...
        $response->assertJsonFragment($testResponse + ['affiliate_id' => $this->getIdFromResponse($response)]);
    }

    private function getIdFromResponse(TestResponse $response)
    {
        return $response->json('affiliate_id') ?? $response->json('data.affiliate_id');
    }
}
